const Deltapath = () => {
  return (
    <div className="bg-[#091424] py-20 text-white px-20">
      <p className="pb-10">
        Deltapath Mobile application & Deltapath Engage can be downloaded easily
        by clicking the icon – once your application is downloaded then you can
        reach to our sales team to provide you the username & password for
        further testing.
      </p>
      <div className="grid grid-cols-3">
        <div className="card w-96 bg-neutral text-neutral-content py-20">
          <div className="card-body items-center text-center">
            <h2 className="card-title">Deltapath Engage</h2>
            <p>Mac App store</p>
            <div className="card-actions justify-end">
              <button className="w-[121px] bg-yellow-500 mr-40 rounded-lg ml-32">
                Download
              </button>
            </div>
          </div>
        </div>
        <div className="card w-96 bg-neutral text-neutral-content py-20">
          <div className="card-body items-center text-center">
            <h2 className="card-title">Deltapath Engage</h2>
            <p>Apple Store</p>
            <div className="card-actions justify-end">
              <button className="w-[121px] bg-yellow-500 mr-40 rounded-lg ml-32">
                Download
              </button>
            </div>
          </div>
        </div>
        <div className="card w-96 bg-neutral text-neutral-content py-20">
          <div className="card-body items-center text-center">
            <h2 className="card-title">Deltapath Engage</h2>
            <p>Play Store</p>
            <div className=" text-center">
              <button className="w-[121px] bg-yellow-500 mr-40 rounded-lg ml-32">
                Download
              </button>
            </div>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-4 ml-20 my-20">
        <div className="card w-2/3 h-36 bg-white text-black">
          <div className="card-body">
            <h2 className="card-title">How webRTC can be...</h2>

            <div className="text-left">
              <span>Read More</span>
            </div>
          </div>
        </div>
        <div className="card w-2/3 h-36 bg-white text-black">
          <div className="card-body">
            <h2 className="card-title">How Deltapath...</h2>

            <div className="text-left">
              <span>Read More</span>
            </div>
          </div>
        </div>
        <div className="card w-2/3 h-36 bg-white text-black">
          <div className="card-body">
            <h2 className="card-title">8 easy SEO Tips For...</h2>

            <div className="text-left">
              <span>Read More</span>
            </div>
          </div>
        </div>
        <div className="card w-2/3 h-36 bg-white text-black">
          <div className="card-body">
            <h2 className="card font-bold">Top 10 digital Marketing...</h2>
            <div className="text-left">
              <span>Read More</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Deltapath;
